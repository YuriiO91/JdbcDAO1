...
public class Main {
    public static void main(String[] args) throws SQLException {
        try (Connection conn = ConnectionFactory.getConnection())
        {
            ...
            Client c = new Client("test", 1, 123333312);
            dao.add(c);
            Client c2 = new Client("test2", 12, 1233333121);
            dao.add(c2);
            // int id = c.getId(); // DZ1
            ...
            // Dz2
            //List<Client> lc = dao.getAll(Client.class, "name", "age");
            //List<Client> lc = dao.getAll(Client.class, "age");
            List<Client> lc = dao.getAll(Client.class, "balance");
            for (Client cli : lc)
                System.out.println(cli);
